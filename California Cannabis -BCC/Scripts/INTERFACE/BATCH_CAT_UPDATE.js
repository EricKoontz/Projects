...
var project = aa.env.getValue("project"); //The project
...
var subject = project + " - CCTT - " + currentEnvironment + " - Success - " + message;

if (debug.indexOf("**ERROR") > 0) {
    aa.env.setValue("returnCode", "-1"); // error
    aa.env.setValue("returnValue", debug);
    subject = project + " - CCTT - " + currentEnvironment + " - Error - " + message;
    aa.sendMail("noreply@accela.com", failureEmail, failureEmailCC, subject, debug);
} else if (aa.env.getValue("returnCode") == "-1") {
    subject = project + " - CCTT - " + currentEnvironment + " - Error - " + message;
    aa.sendMail("noreply@accela.com", failureEmail, failureEmailCC, subject, message);
} else {
    if (showDebug) {
        aa.env.setValue("ScriptReturnDebug", debug);
    }
    if (successEmail) {
        aa.sendMail("noreply@accela.com", successEmail, successEmailCC, subject, message);
    }
}
