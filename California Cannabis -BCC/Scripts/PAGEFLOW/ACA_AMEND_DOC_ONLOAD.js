...
try {
	...
	//skip doc page if not needed on renewal, story 56
	var isSovereignEntity = isASITrue(AInfo["Are they Sovereign Entity"]);

	if (appMatch("Licenses/*/*/Renewal", capId)){
		if (!isSovereignEntity) {
	...
	// get all conditions from parent that are incomplete or applied.   Merge in new requirements from this amendment
	...
	// get docs from the parent record
	...
	// Add conditions for all requirements that don't already have docs, If they are incomplete ignore uploaded docs.
	
	if (r.length > 0) {
		for (x in r) {
			if (uploadedDocs[r[x].document] == undefined) {
				var dr = r[x].condition;
				if (!appHasCondition(conditionType, null, dr, null)) {
