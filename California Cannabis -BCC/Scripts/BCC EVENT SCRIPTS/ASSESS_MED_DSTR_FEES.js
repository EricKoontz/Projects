...
if (vIsApplication == true || vIsAttestation == true) {
	if (vEventName == "ApplicationSubmitAfter") {
		if (vIsApplication) {
			...
			if (vTmp != "Yes") {
				updateFee("MED_DSTR_010", "MED_DSTR_FEE", "FINAL", 1, "Y");
			}
			...
			for (y in vFamilyArray) {
				...
				if (vAltId.substr(2, 3) != "TMP" && vAltId.substr(2, 3) != "EST") {
					vAppFeeExists = feeExists("MED_DSTR_010", "NEW", "INVOICED");
...
if (vIsApplication == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter") {
		if (wfTask == "Supervisory Review" && wfStatus == "Approved") {
			var vFeeCode;
			var vCDFA = getAppSpecific("Cultivator License");
			var vDollar = getAppSpecific("Max dollar value as determined by CDTFA in assessing excise tax");
			if (vDollar != null && vDollar != "" && vCDFA != null && vCDFA != "") {
				if (vCDFA == "Yes") {
					if (vDollar == "Up to 2 million") {
						vFeeCode = "MED_DSTR_052";
					}
					if (vDollar == "Greater than 2 million to 8 million") {
						vFeeCode = "MED_DSTR_053";
					}
				} else if (vCDFA == "No") {
					if (vDollar == "Up to 2 million") {
						vFeeCode = "MED_DSTR_050";
					}
					if (vDollar == "Greater than 2 million to 8 million") {
						vFeeCode = "MED_DSTR_051";
					}
				}
				updateFee(vFeeCode, "MED_DSTR_FEE", "FINAL", 1, "Y");
				...
				resultWorkflowTask("Issuance", "Waiting for Payment", "Update by ASSESS_MED_DSTR_FEES", "Update by ASSESS_MED_DSTR_FEES");
		...
		if (wfTask == "Supervisory Review" && wfStatus == "Temporarily Approved") {
			...
			var vTaskStepNum;
			vTaskStepNum = getTaskStepNumber(vProcessCode, "Issuance", capId);
			resultWorkflowTask("Issuance", "Temporarily Issued", "Update by ASSESS_MED_DSTR_FEES", "Update by ASSESS_MED_DSTR_FEES");
...
if (vIsRenewal == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter" && wfTask == "Supervisory Review" && wfStatus == "Approved") {
		...
		if (vDollar != null && vDollar != "" && vCDFA != null && vCDFA != "") {
			if (vCDFA == "Yes") {
				if (vDollar == "Up to 2 million") {
					vFeeCode = "MED_DSTR_082";
				}
				if (vDollar == "Greater than 2 million to 8 million") {
					vFeeCode = "MED_DSTR_083";
				}
			} else if (vCDFA == "No") {
				if (vDollar == "Up to 2 million") {
					vFeeCode = "MED_DSTR_080";
				}
				if (vDollar == "Greater than 2 million to 8 million") {
					vFeeCode = "MED_DSTR_081";
				}
			}
		...
		if (vLicenseId != null && vLicenseId != false) {
			...
			if (vExpDate < vToday) {
				updateFee("MED_DSTR_070", "MED_DSTR_FEE", "FINAL", 1, "Y");
			}
