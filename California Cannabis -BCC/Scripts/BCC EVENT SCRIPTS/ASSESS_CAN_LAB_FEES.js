...
if (vIsApplication == true || vIsAttestation == true) {
	if (vEventName == "ApplicationSubmitAfter") {
		if (vIsApplication) {
			...
			if (vTmp != "Yes") {
				updateFee("CAN_LAB_010", "CAN_LAB_FEE", "FINAL", 1, "Y");
			}
			...
			for (y in vFamilyArray) {
				...
				if (vAltId.substr(2, 3) != "TMP" && vAltId.substr(2, 3) != "EST") {
					vAppFeeExists = feeExists("CAN_LAB_010", "NEW", "INVOICED");
	...
	if (vEventName == "WorkflowTaskUpdateAfter") {
		if (wfTask == "Supervisory Review" && wfStatus == "Approved") {
			...
			if (vDollar != null && vDollar != "") {
				if (vDollar == "Up to 50 million") {
					vFeeCode = "CAN_LAB_050";
				}
				if (vDollar == "Greater than 50 million to 500 million") {
					vFeeCode = "CAN_LAB_051";
				}
				if (vDollar == "Greater than 500 million") {
					vFeeCode = "CAN_LAB_052";
				}
				updateFee(vFeeCode, "CAN_LAB_FEE", "FINAL", 1, "Y");
		...
		if (wfTask == "Supervisory Review" && wfStatus == "Temporarily Approved") {
			...
			var vTaskStepNum;
			vTaskStepNum = getTaskStepNumber(vProcessCode, "Issuance", capId);
			resultWorkflowTask("Issuance", "Temporarily Issued", "Update by ASSESS_CAN_LAB_FEES", "Update by ASSESS_CAN_LAB_FEES");
...
if (vIsRenewal == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter" && wfTask == "Supervisory Review" && wfStatus == "Approved") {
		...
		if (vDollar != null && vDollar != "") {
			if (vDollar == "Up to 50 million") {
				vFeeCode = "CAN_LAB_080";
			}
			if (vDollar == "Greater than 50 million to 500 million") {
				vFeeCode = "CAN_LAB_081";
			}
			if (vDollar == "Greater than 500 million") {
				vFeeCode = "CAN_LAB_082";
			}
			updateFee(vFeeCode, "CAN_LAB_FEE", "FINAL", 1, "Y");
		}
		...
		if (vLicenseId != null && vLicenseId != false) {
			...
			if (vExpDate < vToday) {
				updateFee("CAN_LAB_070", "CAN_LAB_FEE", "FINAL", 1, "Y");
			}
