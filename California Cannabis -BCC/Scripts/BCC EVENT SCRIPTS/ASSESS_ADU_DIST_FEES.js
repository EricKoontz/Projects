...
if (vIsApplication == true || vIsAttestation == true) {
	if (vEventName == "ApplicationSubmitAfter") {
		if (vIsApplication) {
			...
			if (vTmp != "Yes") {
				updateFee("ADU_DIST_010", "ADU_DIST_FEE", "FINAL", 1, "Y");
			}
			...
			for (y in vFamilyArray) {
				...
				if (vAltId.substr(2, 3) != "TMP" && vAltId.substr(2, 3) != "EST") {
					vAppFeeExists = feeExists("ADU_DIST_010", "NEW", "INVOICED");
...
if (vIsApplication == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter") {
		if (wfTask == "Supervisory Review" && wfStatus == "Approved") {
			...
			if (vDollar != null && vDollar != "") {
				if (vDollar == "Up to 2 million") {
					vFeeCode = "ADU_DIST_050";
				}
				if (vDollar == "Greater than 2 million to 8 million") {
					vFeeCode = "ADU_DIST_051";
				}
				if (vDollar == "Greater than 8 million to 80 million") {
					vFeeCode = "ADU_DIST_052";
				}
				if (vDollar == "Greater than 80 million") {
					vFeeCode = "ADU_DIST_053";
				}
				updateFee(vFeeCode, "ADU_DIST_FEE", "FINAL", 1, "Y");
		...
		if (wfTask == "Supervisory Review" && wfStatus == "Temporarily Approved") {
			...
			var vTaskStepNum;
			vTaskStepNum = getTaskStepNumber(vProcessCode, "Issuance", capId);
			resultWorkflowTask("Issuance", "Temporarily Issued", "Update by ASSESS_ADU_DIST_FEES", "Update by ASSESS_ADU_DIST_FEES");
...
if (vIsRenewal == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter" && wfTask == "Supervisory Review" && wfStatus == "Approved") {
		...
		if (vDollar != null && vDollar != "") {
				if (vDollar == "Up to 2 million") {
					vFeeCode = "ADU_DIST_080";
				}
				if (vDollar == "Greater than 2 million to 8 million") {
					vFeeCode = "ADU_DIST_081";
				}
				if (vDollar == "Greater than 8 million to 80 million") {
					vFeeCode = "ADU_DIST_082";
				}
				if (vDollar == "Greater than 80 million") {
					vFeeCode = "ADU_DIST_083";
				}
			updateFee(vFeeCode, "ADU_DIST_FEE", "FINAL", 1, "Y");
		}
		...
		if (vLicenseId != null && vLicenseId != false) {
			...
			if (vExpDate < vToday) {
				updateFee("ADU_DIST_070", "ADU_DIST_FEE", "FINAL", 1, "Y");
			}
