...
if (vIsApplication == true || vIsAttestation == true) {
	if (vEventName == "ApplicationSubmitAfter") {
		if (vIsApplication) {
			...
			if (vTmp != "Yes") {
				updateFee("ADU_DSTR_010", "ADU_DSTR_FEE", "FINAL", 1, "Y");
			}
			...
			for (y in vFamilyArray) {
				...
				if (vAltId.substr(2, 3) != "TMP" && vAltId.substr(2, 3) != "EST") {
					vAppFeeExists = feeExists("ADU_DSTR_010", "NEW", "INVOICED");
	...
	if (vEventName == "WorkflowTaskUpdateAfter") {
		if (wfTask == "Supervisory Review" && wfStatus == "Approved") {
			...
			if (vDollar != null && vDollar != "" && vCDFA != null && vCDFA != "") {
				...
				updateFee(vFeeCode, "ADU_DSTR_FEE", "FINAL", 1, "Y");
				...
				resultWorkflowTask("Issuance", "Waiting for Payment", "Update by ASSESS_ADU_DSTR_FEES", "Update by ASSESS_ADU_DSTR_FEES");
		...
		if (wfTask == "Supervisory Review" && wfStatus == "Temporarily Approved") {
			...
			var vTaskStepNum;
			vTaskStepNum = getTaskStepNumber(vProcessCode, "Issuance", capId);
			resultWorkflowTask("Issuance", "Temporarily Issued", "Update by ASSESS_ADU_DSTR_FEES", "Update by ASSESS_ADU_DSTR_FEES");
...
if (vIsRenewal == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter" && wfTask == "Supervisory Review" && wfStatus == "Approved") {
		...
		if (vDollar != null && vDollar != "" && vCDFA != null && vCDFA != "") {
			if (vCDFA == "Yes") {
				if (vDollar == "Up to 2 million") {
					vFeeCode = "ADU_DSTR_082";
				}
				if (vDollar == "Greater than 2 million to 8 million") {
					vFeeCode = "ADU_DSTR_083";
				}
			} else if (vCDFA == "No") {
				if (vDollar == "Up to 2 million") {
					vFeeCode = "ADU_DSTR_080";
				}
				if (vDollar == "Greater than 2 million to 8 million") {
					vFeeCode = "ADU_DSTR_081";
				}
			}
		...
		if (vLicenseId != null && vLicenseId != false) {
			...
			if (vExpDate < vToday) {
				updateFee("ADU_DSTR_070", "ADU_DSTR_FEE", "FINAL", 1, "Y");
			}
