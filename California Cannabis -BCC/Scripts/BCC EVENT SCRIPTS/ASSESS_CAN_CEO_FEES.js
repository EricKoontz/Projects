...
if (vIsApplication == true || vIsAttestation == true) {
	if (vEventName == "ApplicationSubmitAfter") {
		if (vIsApplication) {
			...
			if (vTmp != "Yes") {
				updateFee("CAN_CEO_010", "CAN_CEO_FEE", "FINAL", 1, "Y");
			}
			...
			for (y in vFamilyArray) {
				...
				if (vAltId.substr(2, 3) != "TMP" && vAltId.substr(2, 3) != "EST") {
					vAppFeeExists = feeExists("CAN_CEO_010", "NEW", "INVOICED");
...
if (vIsApplication == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter") {
		if (wfTask == "Supervisory Review" && wfStatus == "Approved") {
			var vFeeCode;
			var vDollar = getAppSpecific("How many events do you plan to hold annually?");
			if (vDollar != null && vDollar != "") {
				if (vDollar == "1 - 10 events annually") {
					vFeeCode = "CAN_CEO_050";
				}
				if (vDollar == "Greater than 10 events annually") {
					vFeeCode = "CAN_CEO_051";
				}
				updateFee(vFeeCode, "CAN_CEO_FEE", "FINAL", 1, "Y");
				...
				resultWorkflowTask("Issuance", "Waiting for Payment", "Update by ASSESS_CAN_CEO_FEES", "Update by ASSESS_CAN_CEO_FEES");
		...
		if (wfTask == "Supervisory Review" && wfStatus == "Temporarily Approved") {
			...
			var vTaskStepNum;
			vTaskStepNum = getTaskStepNumber(vProcessCode, "Issuance", capId);
			resultWorkflowTask("Issuance", "Temporarily Issued", "Update by ASSESS_CAN_CEO_FEES", "Update by ASSESS_CAN_CEO_FEES");
...
if (vIsRenewal == true) {
	...
	if (vEventName == "WorkflowTaskUpdateAfter" && wfTask == "Supervisory Review" && wfStatus == "Approved") {
		...
		if (vDollar != null && vDollar != "") {
				if (vDollar == "1 - 10 events annually") {
					vFeeCode = "CAN_CEO_090";
				}
				if (vDollar == "Greater than 10 events annually") {
					vFeeCode = "CAN_CEO_091";
				}
			updateFee(vFeeCode, "CAN_CEO_FEE", "FINAL", 1, "Y");
		}
		...
		if (vLicenseId != null && vLicenseId != false) {
			...
			if (vExpDate < vToday) {
				updateFee("CAN_CEO_070", "CAN_CEO_FEE", "FINAL", 1, "Y");
			}
